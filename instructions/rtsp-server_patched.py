...
def getPipeline(device, height, width, bitrate, format, rotation, framerate):
    ...
    if device == "rpicam":
            ...
            pipeline_str = "( {s_src} ! queue max-size-buffers=1 name=q_enc ! h264parse ! rtph264pay name=pay0 pt=96 config-interval=1 )".format(**locals())      
            ...
            pipeline_str = "( {s_src} ! queue max-size-buffers=1 name=q_enc ! {s_h264} ! rtph264pay name=pay0 pt=96 config-interval=1 )".format(**locals())
